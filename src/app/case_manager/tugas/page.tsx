...
import { Loader2, Building2, Clock, AlertCircle, ChevronRight, Calendar, Unlock } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600">Tugas yang sedang Anda pegang. Klik untuk lanjutkan proses.</p>
      </div>
      ...
      {list.length === 0 ? (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-sm text-slate-500">Tidak ada tugas yang sedang Anda pegang</p>
            <p className="text-xs text-slate-400 mt-1">Cek "Tugas Cabang" untuk ambil tugas baru</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-3">
          {list.map(p => {
            ...
            return (
              <Card key={p.id} className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => router.push(`/case_manager/tugas/detail?id=${p.id}`)}>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {p.sla_deadline && (
                          <span className={`text-xs flex items-center gap-1 ${p.sla_breached || (daysLeft !== null && daysLeft < 0) ? 'text-red-700 font-semibold' : daysLeft !== null && daysLeft <= 3 ? 'text-orange-700' : 'text-slate-500'}`}>
                            ...
                            {daysLeft !== null && daysLeft >= 0 ? `SLA ${daysLeft} hari lagi` : 'SLA lewat'}
                          </span>
                        )}
                        {p.takeover_enabled && (
