...
export default async function KabidApprovalPage() {
  const me = await getSession()
  if (!me) return null
  
  const { data: pipelines } = await supabaseAdmin
    .from('wpa_pipeline')
    .select('id, jenis, current_tahap, sla_deadline, sla_breached, wpa_faskes(nama, jenis, kota)')
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600">Pengajuan yang menunggu approval Anda ({pipelines?.length || 0} antrean).</p>
      </div>
      <Card>
              ...
              {(pipelines || []).length === 0 ? (
                ...
                return (
                  <TableRow key={p.id}>
                    ...
                    <TableCell>
                      <span className={p.sla_breached || (daysLeft !== null && daysLeft < 0) ? 'text-red-700 font-semibold text-xs' : 'text-xs text-slate-500'}>
                        {daysLeft !== null ? `${daysLeft}h` : '-'}
                      </span>
                    </TableCell>
                  </TableRow>
                )
