...
export default async function LegalRsDokumenPage() {
  const me = await getSession()
  if (!me || !me.faskes_id) return null
  
  const { data: pksList } = await supabaseAdmin
    .from('wpa_pks')
    .select('id, kode_pks_pihak_pertama, jenis, status, tanggal_mulai, tanggal_berakhir, signed_at')
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-slate-900">Dokumen Legal</h1><p className="text-sm text-slate-600">Arsip dokumen yang sudah ditandatangani untuk faskes Anda.</p></div>
      <Card><CardContent className="p-0">
        <Table><TableHeader><TableRow>
          <TableHead>Nomor PKS</TableHead><TableHead>Jenis</TableHead><TableHead>Masa Berlaku</TableHead><TableHead>Status</TableHead><TableHead>Tanggal Sign</TableHead>
