...
export function TugasSayaView({ role, currentUserId }: Props) {
  ...
  useEffect(() => {
    async function fetchList() {
      try {
        // Fetch semua tugas di cabang (bukan handler_only, bukan cabang_only)
        // Sistem akan tampilkan semua + status-aware
  ...
  const myTasks = list.filter(p => p.current_handler_id === currentUserId)
  const unclaimedTasks = list.filter(p => p.current_handler_id === null)
  const othersTasks = list.filter(p => p.current_handler_id !== null && p.current_handler_id !== currentUserId)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600">
          {role === 'case_manager' && 'Semua tugas di cabang Anda. Klik "Lanjutkan" untuk tugas yang Anda pegang, atau "Ambil Alih" untuk tugas yang belum dipegang.'}
          ...
          {role === 'kepala_bidang' && 'Semua tugas di cabang. Bisa Ambil Alih kalau perlu, atau buka akses untuk Penata Pelayanan.'}
        </p>
      </div>
...
function TaskCard({ p, status, role, onToggle, toggling, onView }: {
  ...
  const slaClass = p.sla_breached || (daysLeft !== null && daysLeft < 0) ? 'text-red-700 font-semibold' : daysLeft !== null && daysLeft <= 3 ? 'text-orange-700' : 'text-slate-500'
  const slaText = daysLeft !== null && daysLeft >= 0 ? `SLA ${daysLeft}h` : 'SLA lewat'
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {p.sla_deadline && (
                <span className={`text-xs flex items-center gap-1 ${slaClass}`}>
                  <Clock className="w-3 h-3" /> {slaText}
                </span>
              )}
            </div>
          </div>
